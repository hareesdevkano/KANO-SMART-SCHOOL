...
const TokensManagement = () => {
  ...
  const { data: schools } = useQuery({
    queryKey: ["all-schools-for-tokens"],
    queryFn: async () => {
  ...
  const { data: tokens, isLoading } = useQuery({
    queryKey: ["all-tokens", statusFilter, schoolFilter],
    queryFn: async () => {
      let query = supabase
        .from("result_check_tokens")
        .select("*, schools!result_check_tokens_school_id_fkey(name)")
  ...
  const generateMutation = useMutation({
    mutationFn: async ({ schoolId, count }: { schoolId: string; count: number }) => {
      ...
      const { error } = await supabase.from("result_check_tokens").insert(newTokens);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["all-tokens"] });
      setDialogOpen(false);
      toast({ title: "Tokens Generated", description: `${generateCount} tokens created successfully` });
    },
    onError: (error) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (tokenId: string) => {
      const { error } = await supabase.from("result_check_tokens").delete().eq("id", tokenId);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["all-tokens"] });
  ...
  const filteredTokens = tokens?.filter((t) => {
    ...
    return (
      ...
      t.used_by_registration?.toLowerCase().includes(q) ||
  ...
  const estimatedRevenue = unusedTokens * 200; // ₦200 per token
  ...
  const exportTokens = () => {
    ...
    a.href = url;
    a.download = `result-tokens-${format(new Date(), "yyyy-MM-dd")}.csv`;
    a.click();
    ...
    <DashboardLayout role="super-admin">
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-foreground">Result Check Tokens</h1>
            <p className="text-muted-foreground">Generate and manage tokens for student result checking</p>
          </div>
          <div className="flex gap-2">
            ...
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
              ...
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>Generate Result Check Tokens</DialogTitle>
                </DialogHeader>
                <div className="space-y-4 pt-4">
                  ...
                  <div className="rounded-lg bg-muted p-3 text-sm">
                    ...
                    <p className="text-muted-foreground">
                      {generateCount} tokens × ₦200 = <span className="font-semibold text-primary">{formatCurrency(generateCount * 200)}</span> potential revenue
                    </p>
                  </div>
                  <Button
                    className="w-full"
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-bold text-foreground">{formatCurrency(estimatedRevenue)}</p>
          ...
          <CardContent className="pt-6">
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1">
                ...
                <Input
                  placeholder="Search token, reg number, school..."
                  className="pl-10"
                  value={searchQuery}
          ...
          <CardContent>
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="text-sm text-muted-foreground">Generate tokens to get started</p>
              </div>
            ) : (
